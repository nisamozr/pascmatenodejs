jQuery(document).ready(function ($) {
	...
	p = {
		...
		onChange: () => {
			$('#profilImage').on('change', function (e) {
				e.stopPropagation();
				e.preventDefault();
				...
				if (file.length === 1) {
					...
					formData.append('profilImage', file[0]);
	...
	s = {
		...
		onChange: () => {
			$('#coverPhoto').on('change', function (e) {
				...
				if (file.length === 1) {
					let formData = new FormData();
					formData.append('coverPhoto', file[0]);
					console.log(formData, 'formdata');  // this gives me -> FormData {} "formdata"
					s.runAjax(formData);
				}
			});
...
function changePassword() {
	...
	let cpassword = document.getElementById('cfmPassword').value
	let err = document.getElementById('err')
	if (password.length < 8) {
		err.innerHTML = "* Be at least 8 characters *"
		return false
	}
...
function previewPostImage(self) {
	...
	if (file.length > 0) {
		...
		fileReader.onload = function (event) {
			...
			console.log(document.getElementById("post-img-preview").setAttribute("src", event.target.result))
		}
...
function previewPostVideo(self) {
	...
	if (file.length > 0) {
		...
		fileReader.onload = function (event) {
			...
			console.log(document.getElementById('post-video-preview').setAttribute("src", event.target.result))
		}
		fileReader.readAsDataURL(file[0])
	}
...
function doPost(form){
	...
	// formData.append("accessToken",lo)
	ajax.send(formData)
	return false
}

var postimg =document.getElementById("vvv")
if (postimg.src.length== 0) {
